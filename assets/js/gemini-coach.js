...
const JosueCoach = (() => {
    ...
    const DEFAULT_API_KEY = atob(_k1 + _k2 + _k3);
    const MODEL = 'gemini-2.5-flash';

    function getActiveKey() {
        return localStorage.getItem('dma_gemini_api_key') || DEFAULT_API_KEY;
    }

    function getApiUrl() {
        return `https://generativelanguage.googleapis.com/v1beta/models/${MODEL}:generateContent?key=${getActiveKey()}`;
    }
...
- Garde des réponses concises mais complètes (max 300 mots sauf si l'étudiant demande plus de détails)`;
    ...
    async function testConnection(key) {
        try {
            ...
            const response = await fetch(url, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ contents: [{ parts: [{ text: 'Dis "OK"' }] }] })
            });
            const data = await response.json();
            if (!response.ok) {
                return { success: false, message: data.error?.message || `Erreur API (${response.status})` };
            }
            return { success: true };
        } catch (e) {
    ...
    return { chat, resetChat, reset: resetChat, getHistoryLength, testConnection, getActiveKey };
})();
window.JosueCoach = JosueCoach;
