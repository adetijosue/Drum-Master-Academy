...
export const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    const handleSetMetronome = (e: Event) => {
    ...
    window.addEventListener('dma-set-metronome', handleSetMetronome);
    window.addEventListener('dma-log-practice', handleLogPractice);
    return () => {
      window.removeEventListener('dma-set-metronome', handleSetMetronome);
  ...
  const scheduler = () => {
    ...
    while (nextNoteTimeRef.current < audioCtx.currentTime + scheduleAheadTime) {
      ...
      // Calculate Rhythm Presets Mutes/Accents
